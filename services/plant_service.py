...
class PlantService:
    ...
    def get_plants_by_greenhouse(
        ...
        Returns:
            List[Plant]: Lista de plantas ordenadas por fecha de creación
        """
    ...
    def get_all_plants(
        ...
        Returns:
            List[Plant]: Lista de todas las plantas
        """
        return db.query(Plant).order_by(
            Plant.created_at.desc()
        ).offset(skip).limit(limit).all()
    ...
    def update_plant(
        ...
        Args:
            ...
            update_data: Diccionario con los campos a actualizar
                        Ejemplo: {"name": "Nuevo nombre", "type": "Nuevo tipo"}
    ...
    @staticmethod
    def count_plants_by_greenhouse(
            db: Session,
            greenhouse_id: int
    ) -> int:
        """
        Cuenta el número de plantas en un invernadero
        ...
        Returns:
            int: Número de plantas en el invernadero
        """
        return db.query(Plant).filter(
            Plant.greenhouse_id == greenhouse_id
        ).count()
    ...
    def get_plants_by_type(
        ...
        Args:
            db: Sesión de base de datos
            plant_type: Tipo de planta a buscar
            skip: Número de registros a saltar
            limit: Número máximo de registros a retornar

        Returns:
            List[Plant]: Lista de plantas del tipo especificado
        """
        return db.query(Plant).filter(
    ...
    def search_plants_by_name(
        ...
        """
        Busca plantas por nombre (búsqueda parcial, case-insensitive)
        ...
        Returns:
            List[Plant]: Lista de plantas que coinciden con la búsqueda
        """
        return db.query(Plant).filter(
    ...
    @staticmethod
    def bulk_delete_plants_by_greenhouse(
            db: Session,
            greenhouse_id: int
    ) -> int:
        """
        Elimina todas las plantas de un invernadero
        ...
        Returns:
            int: Número de plantas eliminadas
        """
        try:
